...
import { Plus, Pencil, Trash2, Save, X, Loader2, Users, UserCog } from "lucide-react";
...
export default function UsersPage() {
  ...
  const qcCount = users.filter(u => u.role !== "super_admin").length;

  return (
    <div className="space-y-5">
      {/* Hero */}
      <HeroBanner icon={UserCog} title="User Management" subtitle={`${users.length} user — ${superAdminCount} admin, ${qcCount} QC`} />

      {/* Header */}
      <PageHeader title="Daftar User" count={users.length}>
      ...
      {showForm && (
        <CollapsibleSection title={editingId ? "Edit User" : "Tambah User Baru"} icon={editingId ? Pencil : Plus} defaultOpen={true}>
      ...
      {loading ? <SkeletonLoader rows={4} type="list" /> : users.length === 0 ? (
        ...
        <>
          ...
          <Card className="hidden sm:block overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm font-sans">
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} className="border-b border-[#4FD1FF]/8 hover:bg-[#4FD1FF]/[0.02] transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#4FD1FF]/10 to-[#9F7AEA]/10 flex items-center justify-center text-xs font-bold text-[#4FD1FF]">
                            {u.username.charAt(0).toUpperCase()}
                          </div>
                          <span className="text-[#E5E7EB] font-medium">{u.username}</span>
                        </div>
                      </td>
          ...
          <div className="sm:hidden space-y-3">
            {users.map((u) => (
              <Card key={u.id} hover className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 mb-1">
                      <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#4FD1FF]/10 to-[#9F7AEA]/10 flex items-center justify-center text-xs font-bold text-[#4FD1FF] shrink-0">
                        {u.username.charAt(0).toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-sans text-[#E5E7EB] font-medium">{u.username}</p>
                        <p className="text-[11px] font-sans text-[#4FD1FF]/60 truncate">{getTenantName(u.tenant_id)}</p>
                      </div>
                    </div>
