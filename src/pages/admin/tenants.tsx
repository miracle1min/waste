...
import { Plus, Pencil, Trash2, Save, X, Loader2, Store, Building2 } from "lucide-react";
...
export default function TenantsPage() {
  ...
  const ownDbCount = tenants.filter(t => t.neon_database_url).length;

  return (
    <div className="space-y-5">
      {/* Hero */}
      <HeroBanner icon={Building2} title="Store Management" subtitle={`Kelola ${tenants.length} store — ${activeCount} aktif, ${ownDbCount} own DB`} />

      {/* Header */}
      <PageHeader title="Daftar Store" count={tenants.length} subtitle="Tambah, edit, atau hapus store">
      ...
      {/* Form */}
      {showForm && (
        <CollapsibleSection title={editingId ? "Edit Store" : "Tambah Store Baru"} icon={editingId ? Pencil : Plus} defaultOpen={true}>
      ...
      {/* List */}
      {loading ? <SkeletonLoader rows={4} type="list" /> : tenants.length === 0 ? (
          ...
          <Card className="hidden sm:block overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm font-sans">
                ...
                <tbody>
                  {tenants.map((t) => (
                    <tr key={t.id} className="border-b border-[#4FD1FF]/8 hover:bg-[#4FD1FF]/[0.02] transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="relative">
                            <div className={`w-2 h-2 rounded-full ${t.status === "active" ? "bg-green-400" : "bg-red-400"}`} />
                            {t.status === "active" && <div className="absolute inset-0 w-2 h-2 rounded-full bg-green-400 animate-ping opacity-30" />}
                          </div>
                          <div>
