...
export default function PdfDownload() {
 ...
 {/* Desktop header */}
 <div className="hidden lg:flex items-center gap-3 px-6 py-4 border-b border-[#4FD1FF]/10 bg-[hsl(220,45%,8%)]/80 backdrop-blur-sm">
 <FileText className="w-5 h-5 text-[#4FD1FF]" />
 <h1 className="text-lg font-bold text-[#4FD1FF]">Generate PDF</h1>
 <span className="text-xs text-[#9CA3AF]">BA Waste Report</span>
 {tenantName && <span className="text-xs text-[#4FD1FF]/80/70 font-sans ml-2">{tenantName}</span>}
 </div>

 {/* Mobile header */}
 <header className="sticky top-0 z-50 border-b border-[#4FD1FF]/10 bg-[hsl(220,45%,8%)]/95 backdrop-blur-md lg:hidden">
 ...
 <FileText className="w-3.5 h-3.5 text-[#4FD1FF] flex-shrink-0" />
 Generate PDF
 </h1>
 ...
 <div className="hidden sm:flex items-center gap-1.5 px-2 py-1 rounded-md bg-[#4FD1FF]/8 border border-[#4FD1FF]/10">
 ...
 <label className="text-[10px] font-medium text-[#9CA3AF] mb-1 block">📅 Pilih Bulan</label>
 <select
 value={selectedMonth}
 onChange={(e) => { setSelectedMonth(e.target.value); }}
 className="w-full bg-slate-900 border border-[#4FD1FF]/15 rounded-lg px-3 py-2.5 text-sm text-white focus:border-[#4FD1FF]/30 focus:ring-1 focus:ring-[#4FD1FF]/30 outline-none"
 >
 ...
 <span className="text-[10px] text-[#9CA3AF] block">{availableDates.length} total</span>
 {selectedPdfDates.size > 0 && (
 <span className="text-[10px] text-[#4FD1FF] font-semibold">{selectedPdfDates.size} dipilih</span>
 )}
 </div>
 </div>

 {/* Main PDF section */}
 <div className="bg-[hsl(220,45%,10%)] border border-[#4FD1FF]/10 rounded-xl p-4">
 ...
 className="text-[10px] text-[#4FD1FF] hover:bg-[#4FD1FF]/8 px-2 h-7"
 >
 ...
 {selectedInMonth > 0 && (
 <div className="text-[10px] text-[#4FD1FF] mb-2">{selectedInMonth}/{filteredDates.length} dipilih bulan ini</div>
 )}

 {loading ? (
 <div className="flex items-center justify-center py-10">
 <Loader2 className="w-6 h-6 animate-spin text-[#4FD1FF] mr-2" />
 <span className="text-[#9CA3AF] text-sm">Loading tanggal...</span>
 </div>
 ) : availableDates.length > 0 ? (
 <>
 {/* Date list for selected month */}
 {!selectedMonth ? (
 <p className="text-center text-[#9CA3AF] text-sm py-6">👆 Pilih bulan dulu di dropdown atas</p>
 ) : filteredDates.length === 0 ? (
 <p className="text-center text-[#9CA3AF] text-sm py-6">Tidak ada data di bulan ini</p>
 ) : (
 ...
 isSelected
 ? "border-cyan-500/60 bg-cyan-950/40 text-[#4FD1FF]"
 : "border-[#4FD1FF]/10 bg-[hsl(220,45%,8%)] text-[#9CA3AF] hover:border-[#4FD1FF]/15 hover:bg-[hsl(220,45%,9%)]"
 } ${pdfGenerating ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
 >
 {isSelected ? (
 <CheckSquare className="w-4 h-4 text-[#4FD1FF] flex-shrink-0" />
 ) : (
 <Square className="w-4 h-4 text-[#6B7280] flex-shrink-0" />
 )}
 <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center flex-shrink-0">
 <span className="text-sm font-bold text-[#E5E7EB]">{dayNum}</span>
 </div>
 <div className="text-left flex-1">
 <span className="font-medium">{dayName}</span>
 <span className="text-[#9CA3AF] ml-2 text-xs font-sans">{display}</span>
 </div>
 <FileText className={`w-3.5 h-3.5 flex-shrink-0 ${isSelected ? 'text-[#4FD1FF]/80' : 'text-[#6B7280]'}`} />
 </button>
 );
 })}
 </div>
 ...
 <div className="flex items-center gap-2 text-xs text-[#4FD1FF]">
 <Loader2 className="w-3.5 h-3.5 animate-spin flex-shrink-0" />
 <span className="truncate">{pdfProgress || 'Mempersiapkan...'}</span>
 <span className="ml-auto font-sans font-bold text-[#4FD1FF] flex-shrink-0">
 {pdfProgressNum.current}/{pdfProgressNum.total}
 </span>
 </div>
 <div className="w-full bg-slate-900 rounded-full h-2.5 overflow-hidden">
 <div
 className="h-full rounded-full bg-gradient-to-r from-[#4FD1FF] via-[#9F7AEA] to-[#9F7AEA] transition-all duration-700 ease-out"
 ...
 <p className="text-[10px] text-[#9CA3AF] text-center">⏳ Mohon tunggu, sedang memproses foto dokumentasi...</p>
 </div>
 )}
 ...
 className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:border-[#4FD1FF]/30 focus:ring-1 focus:ring-[#4FD1FF]/30 outline-none"
 >
 ...
 className="w-full bg-gradient-to-r from-[#4FD1FF] to-[#9F7AEA] hover:from-[#4FD1FF]/80 hover:to-[#9F7AEA]/80 text-white font-semibold py-2.5 disabled:opacity-50"
 >
 {pdfGenerating ? (
 <>
 ...
 <p className="text-center text-[#9CA3AF] py-6">Belum ada data buat generate PDF nih</p>
 )}
 </div>
 ...
 💡 <strong className="text-[#9CA3AF]">Tips:</strong> Pilih beberapa tanggal sekaligus untuk batch download. 
